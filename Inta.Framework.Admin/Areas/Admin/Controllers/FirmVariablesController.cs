...
namespace Inta.Framework.Admin.Areas.Admin.Controllers
{
    ...
    public class FirmVariablesController : Controller
    {
        ...
        public ActionResult GetList(PagingDataListRequest<FirmVariablesSearch> request)
        {
            ...
            string sqlQuery = "Select * from FirmVariables where (@Name is null or Name like '%'+@Name+'%') and  (@IsActive is null or IsActive=@IsActive) order by " + request.OrderColumn + (request.OrderType == PagingDataListOrderType.Ascending ? " asc" : " desc");
            ...
            var selectData = pagingData.Select(s => new
            {
                ...
                Edit = "<a href='javascript:void(0)' onclick=\"$PagingDataList.AddRecordModal('/Admin/FirmVariables/Add','True'," + s.Id.ToString() + ")\"><img src='/Areas/Admin/Content/images/edit-icon.png' width='20'/></a>",
                Delete = "<a href='javascript:void(0)' onclick=\"$PagingDataList.DeleteRecordModal('PagingDataList','/Admin/FirmVariables/Delete',SearchDataList," + s.Id.ToString() + ")\"><img src='/Areas/Admin/Content/images/delete-icon.png' width='20'/></a>"
            }).ToList();
