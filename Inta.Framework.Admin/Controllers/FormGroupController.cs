...
namespace Inta.Framework.Admin.Controllers
{
    ...
    public class FormGroupController : Controller
    {
        ...
        public ActionResult GetList(PagingDataListRequest<FormGroupSearch> request)
        {
            ...
            string sqlQuery = "Select * from FormGroup where (@Name is null or Name like '%'+@Name+'%') and IsActive=@IsActive order by " + request.OrderColumn + (request.OrderType == PagingDataListOrderType.Ascending ? " asc" : " desc");
            ...
            var selectData = pagingData.Select(s => new
            {
                ...
                Edit = "<a href='javascript:void(0)' onclick=\"$PagingDataList.AddRecordModal('/FormGroup/Add','True'," + s.Id.ToString() + ")\">Düzenle</a>",
                Delete = "<a href='javascript:void(0)' onclick=\"$PagingDataList.DeleteRecordModal('BannerTypeList','/FormGroup/Delete',SearchDataList," + s.Id.ToString() + ")\">Sil</a>"
            }).ToList();
