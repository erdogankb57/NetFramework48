...
namespace Inta.Framework.Admin.Controllers
{
    ...
    public class StaticTextController : Controller
    {
        ...
        public ActionResult GetList(PagingDataListRequest<StaticText> request)
        {
            ...
            string sqlQuery = "Select * from StaticText where (@Name is null or Name like '%'+@Name+'%') and IsActive=@IsActive order by " + request.OrderColumn + (request.OrderType == PagingDataListOrderType.Ascending ? " asc" : " desc");
            ...
            var selectData = pagingData.Select(s => new
            {
                ...
                Edit = "<a href='javascript:void(0)' onclick=\"$PagingDataList.AddRecordModal('/StaticText/Add','True'," + s.Id.ToString() + ")\">Düzenle</a>",
                Delete = "<a href='javascript:void(0)' onclick=\"$PagingDataList.DeleteRecordModal('PagingDataList','/StaticText/Delete',SearchDataList," + s.Id.ToString() + ")\">Sil</a>"
            }).ToList();
