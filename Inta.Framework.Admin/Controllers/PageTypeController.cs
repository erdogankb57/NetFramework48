...
namespace Inta.Framework.Admin.Controllers
{
    ...
    public class PageTypeController : Controller
    {
        ...
        public ActionResult GetList(PagingDataListRequest<PageType> request)
        {
            ...
            string sqlQuery = "Select * from PageType where (@Name is null or Name like '%'+@Name+'%') and IsActive=@IsActive order by " + request.OrderColumn + (request.OrderType == PagingDataListOrderType.Ascending ? " asc" : " desc");
            ...
            var selectData = pagingData.Select(s => new
            {
                ...
                Id = s.Id,
                Name = s.Name,
                Edit = "<a href='javascript:void(0)' onclick=\"$PagingDataList.AddRecordModal('/PageType/Add','True'," + s.Id.ToString() + ")\">Düzenle</a>",
                Delete = "<a href='javascript:void(0)' onclick=\"$PagingDataList.DeleteRecordModal('PageTypeList','/PageType/Delete',SearchDataList," + s.Id.ToString() + ")\">Sil</a>"
            }).ToList();
